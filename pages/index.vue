...
<template>
  <div dir="rtl" class="home-page bg-white text-secColor">
    ...
    <!-- هیرو -->
    <section class="relative pt-32 pb-24 px-4 overflow-hidden bg-no-repeat bg-cover bg-bottom"
      style="background: url('/images/back.jpg');">
      ...
      <div class="relative z-10 text-center max-w-2xl mx-auto">
        ...
        <div class="flex justify-center gap-3 mb-10">
          ...
          <NuxtLink to="/contact">
            <UButton variant="outline" color="white" class="rounded-full font-bold px-6 text-white border-white/40">تماس
              با ما</UButton>
          </NuxtLink>
        </div>
      </div>
    ...
    <main>
        ...
        <UContainer class="py-14 md:py-16">
          <div class="section-head">
            <div>
              ...
              <h2 class="section-title">همه خودروها را ببین</h2>
            </div>
            <NuxtLink to="/cars" class="section-link">مشاهده همه ←</NuxtLink>
          </div>
          <HomeTab />
      ...
      <UContainer class="py-14 md:py-16">
          ...
          <div class="grid md:grid-cols-2">
            <div class="relative min-h-[310px] overflow-hidden">
              ...
              <span class="absolute inset-0 flex items-center justify-center"><span
                  class="flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-xl">
                  <UIcon name="mdi:play" class="h-6 w-6 text-secColor" />
                </span></span>
            </div>
            <div class="flex flex-col justify-center p-7 md:p-12">
              <p class="section-kicker">SELL YOUR CAR</p>
              <h2 class="text-2xl font-black leading-10 text-secColor md:text-3xl">قیمت واقعی خودروی خودت را پیدا کن
              </h2>
              <p class="mt-4 text-sm leading-7 text-gray-500">مشخصات خودرو را برای ما ارسال کن تا برای فروش و قیمت‌گذاری
                راهنماییت کنیم.</p>
              <ul class="mt-5 space-y-3 text-sm text-gray-600">
                <li class="flex items-center gap-2">
                  <UIcon name="mdi:check-circle" class="text-mainColor" /> کارشناسی و بررسی دقیق
                </li>
                <li class="flex items-center gap-2">
                  <UIcon name="mdi:check-circle" class="text-mainColor" /> پیشنهاد قیمت شفاف
                </li>
                <li class="flex items-center gap-2">
                  <UIcon name="mdi:check-circle" class="text-mainColor" /> پشتیبانی تا پایان معامله
                </li>
              </ul>
              <NuxtLink to="/contact" class="mt-7 inline-flex w-fit">
                <UButton color="yellow" class="rounded-xl px-6 font-bold">فروش خودرو</UButton>
              </NuxtLink>
            </div>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-5 py-10 md:grid-cols-4">
          <div v-for="stat in stats" :key="stat.label" class="text-center">
            <p class="text-2xl font-black md:text-3xl">{{ stat.value }}</p>
      ...
      <section class="border-y border-gray-100">
        <UContainer class="py-14 md:py-16">
          <div class="mb-9">
            <p class="section-kicker">WHY CARPAZ</p>
            <h2 class="section-title">چرا کارپاز؟</h2>
          </div>
          <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            <div v-for="reason in reasons" :key="reason.title">
              <div class="reason-icon">
                <UIcon :name="reason.icon" class="h-6 w-6" />
              </div>
        ...
        <UContainer class="py-14 md:py-16">
          <div class="section-head">
            <div>
              ...
              <h2 class="text-2xl font-black text-white md:text-3xl">خودروهای پرطرفدار</h2>
            </div>
            <NuxtLink to="/cars" class="section-link">مشاهده همه ←</NuxtLink>
          </div>
          <div v-if="cars.length" class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <CarCard v-for="car in cars.slice(0, 4)" :key="car.id" :car="car" />
          </div>
          <div v-else class="rounded-2xl border border-white/10 py-12 text-center text-sm text-white/50">خودرویی برای
            نمایش وجود ندارد.</div>
        </UContainer>
      </section>
      ...
      <UContainer class="py-14 md:py-16">
        <div class="section-head">
          <div>
            <p class="section-kicker">SHOP BY MAKE</p>
            <h2 class="section-title">خودرو را بر اساس برند پیدا کن</h2>
          </div>
          <NuxtLink to="/cars" class="section-link">مشاهده همه ←</NuxtLink>
        </div>
        <div class="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-4">
          <NuxtLink v-for="brand in brands.slice(0, 8)" :key="brand.id" :to="`/cars?brand=${brand.id}`"
            class="make-link">{{ brand.name }} <span>←</span></NuxtLink>
        </div>
      </UContainer>
        ...
        <UContainer class="py-14 md:py-16">
          <div class="section-head">
            <div>
              ...
              <h2 class="section-title">مشتری‌های ما چه می‌گویند؟</h2>
            </div><span class="hidden text-xs text-gray-400 md:block">تجربه واقعی مشتریان کارپاز</span>
          </div>
          <div class="grid items-center gap-8 md:grid-cols-[180px_1fr]">
            ...
            <div>
              <div class="mb-3 text-mainColor">★★★★★</div>
              <p class="text-sm leading-8 text-gray-600 md:text-base">تجربه خرید برای من خیلی راحت‌تر از چیزی بود که فکر
                می‌کردم. اطلاعات خودرو کامل بود و تیم کارپاز در تمام مراحل پاسخگو بود.</p>
              <p class="mt-5 font-extrabold">مشتری کارپاز</p>
              <p class="mt-1 text-xs text-gray-400">خریدار خودرو</p>
            </div>
          </div>
        </UContainer>
      </section>
      ...
      <UContainer class="py-14 md:py-16">
        <div class="section-head">
          <div>
            ...
            <h2 class="section-title">آخرین مطالب خودرو</h2>
          </div>
          <NuxtLink to="/about" class="section-link">مشاهده همه ←</NuxtLink>
        </div>
        <div class="grid gap-5 md:grid-cols-3">
          <article v-for="post in posts" :key="post.title"
            ...
            <div class="p-5"><span class="text-[11px] font-bold text-mainColor">{{ post.tag }}</span>
              <h3 class="mt-2 font-extrabold leading-7">{{ post.title }}</h3><span
                class="mt-4 inline-flex text-xs font-bold text-gray-400">ادامه مطلب ←</span>
            </div>
          </article>
        </div>
      </UContainer>
      ...
      <UContainer class="pb-14 md:pb-16">
        <div class="grid gap-5 md:grid-cols-2">
          <div class="cta-card bg-[#edf4ff]">
            <div>
              <p class="text-xs font-bold text-mainColor">خریدار خودرو هستی؟</p>
              <h3 class="mt-2 text-xl font-black">خودروی بعدی‌ات منتظر توست</h3>
              <p class="mt-2 text-xs leading-6 text-gray-500">بین آگهی‌ها جستجو کن و گزینه مناسب خودت را پیدا کن.</p>
              <NuxtLink to="/cars" class="mt-5 inline-flex">
                <UButton color="yellow" class="rounded-xl font-bold">جستجوی خودرو</UButton>
              </NuxtLink>
            </div>
            <UIcon name="mdi:car-search" class="h-14 w-14 shrink-0 text-mainColor" />
          </div>
          <div class="cta-card bg-[#fff0f5]">
            <div>
              <p class="text-xs font-bold text-mainColor">قصد فروش داری؟</p>
              <h3 class="mt-2 text-xl font-black">خودروتو به کارپاز بسپار</h3>
              <p class="mt-2 text-xs leading-6 text-gray-500">برای فروش و قیمت‌گذاری خودرو با ما در تماس باش.</p>
              <NuxtLink to="/contact" class="mt-5 inline-flex">
                <UButton color="yellow" class="rounded-xl font-bold">فروش خودرو</UButton>
              </NuxtLink>
            </div>
            <UIcon name="mdi:cash-check" class="h-14 w-14 shrink-0 text-mainColor" />
          </div>
        </div>
      </UContainer>
    </main>
...
.brand-card:hover {
  ...
  box-shadow: 0 10px 25px rgba(34, 40, 49, .07);
}
